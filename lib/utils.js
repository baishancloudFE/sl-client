module.exports = (function() {
  ...
  _exports.connect = function() {
    ...
    if (!server) {
      console.warn(chalk.red('can\'t get the sl server info!\n'))
      console.warn(chalk.cyanBright('please set the sl server:\n    sl --set-server=[host]:[port]\n'))
      throw new Error('can\'t get the server info.')
    }

    let _resolve
    const connected = new Promise(resolve => _resolve = resolve)

    const socket = require('net').createConnection(server, () => {
